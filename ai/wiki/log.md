...
## [2026-08-20] finding | Act III could be locked dead. act3Tick is wrapped in `if(s.spores>0)`, and the engagement with wild yeast subtracted from the fleet AFTER the never-zero floor was applied, so a fleet taken to exactly zero froze the entire act — no exploration, no conversion, and no decay of the colonies that did it, for ever. Rogue colonies meanwhile compounded 2%/s against nothing and reached thirteen sextillion. The escape, the Wild Yeast recipe, cost 90,000 inspiration against an Act III ceiling of 40,000 for a player arriving with one notebook: visible, greyed out, and unreachable. Entered by putting a third point into Self-replication, a trait described only as "spores making further spores".

## [2026-08-20] fix | The floor is held after the engagement as well as before; colonies starve at 8%/s when they take nothing; colonies are capped at twice the fleet they came from; a raid can take at most half a second's worth of the fleet per tick, so a reseed is not eaten before it lands; Wild Yeast repriced 90,000 -> 30,000, under the ceiling a player reaches the act with. Played out in a browser: stalled at one spore against forty colonies, bought the recipe, moved three points into Defence, relaunched, and was back to 19,385 spores and 1.01% converted thirty minutes later.

## [2026-08-20] fix | The Self-replication row now warns at two points that copies are about to start going wrong and says plainly at three that some stop answering. The panel's advice line drops everything else while a raid is running and names the answer. This was the one point in the panel that could cost a player the act, and nothing said so.

## [2026-08-20] tool | ai/tools/play.js committed. Needs Chrome and playwright-core, neither of which the game itself depends on — it still runs from disk with no build step and no packages. Set CHROME, URL and SHOTS to taste.
